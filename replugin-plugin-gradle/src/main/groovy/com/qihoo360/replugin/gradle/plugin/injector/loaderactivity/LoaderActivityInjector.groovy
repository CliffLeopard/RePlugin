...
public class LoaderActivityInjector extends BaseInjector {
    ...
    private def handleActivity(ClassPool pool, String activity, String classesDir) {
        ...
        def clsFile = new File(classesDir, activity.replaceAll('\\.', '/') + '.class')
        if (!clsFile.exists()) {
            return
        }
        ...
        def stream, ctCls
        try {
            stream = new FileInputStream(clsFile)
